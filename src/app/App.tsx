...
function AdminRoute() {
  ...
  // If authenticated, show the Shell. Otherwise, show the Auth screen.
  if (isAuth) {
...
function AppContent() {
  ...
  const handleNavigate = (f: Filter | null) => {
    ...
    if (location.pathname !== "/") {
      navigate("/");
      // Wait for React to render the Storefront before scrolling
      setTimeout(() => {
  ...
  return (
    <div className="min-h-screen bg-white text-foreground" style={{ fontFamily: "var(--font-body)" }}>
      ...
      {/* Global Overlays (Cart & Product Viewer) - Hidden on Admin */}
      {!isAdminRoute && (
        <>
          {activeProduct && (
            <ProductPage
